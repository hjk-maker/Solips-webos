      ...
      function showFileTypes(){
        content.innerHTML = `<h2>File Types</h2>
          <div class="set-info">
            ...
            <br><br>
            <strong>Only Editor and Browser actually load the file's contents</strong>
            (Browser renders .html/.htm as a real page, everything else as text).
            Pointing an extension at any other app — Terminal, Settings, Store,
            or a custom app you've added — will still open that app, but it
            won't receive the file automatically, since most apps have no way
            to accept one. That's not a bug in the setting; it's a limit of
            what that app knows how to do with a file.
          </div>
          <h3>Add / Override</h3>
...
function makeBrowserApp(fs){
  return {
    ...
    init(container, winObj, context){
      ...
      // Launched via a file association (e.g. .html mapped to Browser): load
      // the actual file content through a Blob URL so this is a real local
      // preview, not just an empty browser window that ignores what you clicked.
      container._openFile = async (path) => {
        ...
        frame.src = blobUrl;
        urlInput.value = path;
        externalLink.href = blobUrl;
        if (winObj) winObj.title = path.split('/').pop();
      };
