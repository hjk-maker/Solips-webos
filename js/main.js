...
(async function boot(){
  ...
  try {
    ...
    const filesApp = makeFilesApp(fs, (path) => {
      // openApp's context param (not a setTimeout guess) is what actually
      // gets the clicked file into the target app, whether it's opening
      // fresh or already open. Apps that can't do anything with a file
      // (custom iframe apps, Store, etc.) just ignore the context — they
      // still open, they just won't magically preview something they were
      // never built to read.
      window._OS.openApp(fileAssoc.appFor(path), { filePath: path });
    });
    const settingsApp = makeSettingsApp(ai, net);
    const browserApp = makeBrowserApp(fs);
    ...
    window._OS = {
      ...
      openApp(id, context) {
        ...
        const alreadyOpen = wm.windows.has(id);
        wm.create({
          id: app.id,
          title: app.name,
          icon: app.icon,
          ...
          content: (body, winObj) => {
            const result = app.init(body, winObj, context);
            return result;
          }
        });
        // wm.create() reuses an already-open window instead of duplicating it,
        // which means init() above did NOT run again — so a fresh file context
        // would otherwise silently never reach it. Push it in directly.
        if (alreadyOpen && context?.filePath){
          const win = wm.windows.get(id);
          const body = win?.el.querySelector('.win-body');
          if (body?._openFile) body._openFile(context.filePath);
        }
      },
      ...
      async executeTool(name, params){
        switch(name){
          ...
          case 'app_open': window._OS.openApp(params.id, params.filePath ? { filePath: params.filePath } : undefined); return 'opened';
    ...
    ctxMenu.addEventListener('click', (e) => {
      ...
      switch(act){
        case 'newFile': { const n = prompt('File name:', '/untitled.txt'); if (n){ fs.write(n, ''); window._OS.openApp('editor', { filePath: n }); } break; }
